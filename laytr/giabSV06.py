...
def make_giabreport(in_dir):
    """
    Create summaries of the TPs/FNs
    """
    out_df = os.path.join(in_dir, "giab.jl")
    data = truvari.vcf_to_df(in_dir)
    stats_box = json.load(open(os.path.join(in_dir, "summary.json"), 'r'))
    ...
    make_tech(data)
    with open(os.path.join(in_dir, "giab_report.txt"), 'w') as sum_out:
...
def parse_args(args):
    ...
    Argument parser
    """
    parser = argparse.ArgumentParser(prog="giab_SV0.6", description=__doc__,
                                     formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("-i", "--input", required=True,
                        help="Truvari result directory")
    return parser.parse_args(args)

def giabSV06_main(args):
    """
    Main
    """
    args = parse_args(args)
    make_giabreport(args.input)
